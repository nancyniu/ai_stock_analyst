...
### initialize 
if 'value_count' not in st.session_state:
    st.session_state.value_count = 0
...
def update():
    ...
    if st.session_state.value_count == 0:
        st.write(st.session_state.count_value)
        df = get_screener_symbols()
        edited_df = st.data_editor(df, disabled=["Ticker", "Company"], hide_index=True,)
        st.session_state.saved_df = edited_df
        st.session_state.count_value += 1
    elif st.session_state.value_count > 1:
        st.write(st.session_state.count_value)
...
if st.button("scan", type="primary", on_click=update()):
    st.session_state.value_count += 1
...
if st.button("Go", type="primary"):
    edited_df = st.session_state.saved_df
